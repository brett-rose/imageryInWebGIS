define(["dojo/_base/declare", "dijit/_WidgetBase", "dojox/gfx","dojo/_base/array", "dojo/has", "dojo/has!dojo-bidi?../bidi/widget/Legend",
	...
	var Legend = declare(has("dojo-bidi")? "dojox.charting.widget.NonBidiLegendExt" : "dojox.charting.widget.LegendExt", _WidgetBase, {
		// summary:
		...
		refresh: function(){
			...
			if(s[0].chart.stack[0].declaredClass == "dojox.charting.plot2d.Pie"){
			...
			}else if(s[0].chart.stack[0].declaredClass == "dojox.charting.plot2d.Rose"){

			  var t = s[0].chart.stack[0];
			  var slices = t.run.frequencyData;// df.map(filteredRun, "/this", df.foldl(filteredRun, "+", 0));
			  var lbl, freqs = [], freq;
			  ...
			  var total = freqs.reduce((a,b)=> a+b);
			  console.log(total);
			  labels = arr.forEach(freqs, function (x, i) {
			    if (x > 0) {
			      //return x > 0 ? this._getLabel(x * 100) + "%" : "";
			      lbl = " level " + i + " "+t._getLabel(x * 100 / total) + "%";
			      this._addLabel(t.dyn[i], lbl);
			    }
			  }, this);
			}else{
